...
namespace :bindings do
  ...
  task :'enable-functions' do
    sh 'mrbind enable-functions -m UI -g mruby-ui -o .'
  end
  
  task :fn_count do
    sh "cat include/mruby_UI_functions.h | egrep 'TRUE|FALSE' | wc -l"
  end

  task :bound_fns do
    sh "cat include/mruby_UI_functions.h | egrep 'TRUE'"
  end

  task :bound_fn_count do
    sh "cat include/mruby_UI_functions.h | egrep 'TRUE' | wc -l"
  end

  task :unbound_fns do
    sh "cat include/mruby_UI_functions.h | egrep 'FALSE'"
  end

  task :unbound_fn_count do
    sh "cat include/mruby_UI_functions.h | egrep 'FALSE' | wc -l"
  end
end
